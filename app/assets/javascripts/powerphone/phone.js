 ...
 * Modified file:
 *   By: Matthieu CIAPPARA for Modulotech Group
 *   Date: April 2024
 *   Git: https://github.com/modulotech/powerphone
 */
 ...
 */
// Directory For Image Assets eg: images/
...
// Un-registers this account if the phone is opened in another tab/window
...
// Enable Subscribe to your own uri. (Useful to understand how other buddies see you.)
let SubscribeToYourself = getDbItem('SubscribeToYourself', '0') === '1'
// Enable Subscribe to voicemail
let VoiceMailSubscribe = getDbItem('VoiceMailSubscribe', '1') === '1'
// Number to dial for VoicemialMain()
...
// Optional name for contact header uri
...
// Use if hosting off root directory. eg: "/phone/" or "/static/"
...
// Parsable Json string of headers to include in register process. eg: '{"foo":"bar"}'
let RegisterExtraHeaders = getDbItem('RegisterExtraHeaders', '{}')
// Parsable Json string of extra parameters add to the end (after >) of contact header during register. eg: '{"foo":"bar"}'
let RegisterExtraContactParams = getDbItem('RegisterExtraContactParams', '{}')
// Parsable Json string of extra parameters added to contact URI during register. eg: '{"foo":"bar"}'
...
// Set a random IP address as the host value in the Contact header field and Via sent-by parameter. (Suggested for Asterisk)
...
// Set amount of time in milliseconds to wait for the ICE/STUN server
...
// For application/pidf+xml use presence. For application/dialog-info+xml use dialog
let SubscribeBuddyEvent = getDbItem('SubscribeBuddyEvent', 'presence')
// Buddy Subscription expiry time (in seconds)
let SubscribeBuddyExpires = parseInt(getDbItem('SubscribeBuddyExpires', 300))
// Can display an item from your vCard before your name. Options: Number1 | Number2
...
// Extra SIP headers to be included in the initial INVITE message for each call. (Added to the extra headers in the DialByLine() parameters. e.g {"foo":"bar"})
...
// Automatically answers the phone when the call comes in, if you are not on a call already
...
// Rejects any inbound call if you are on a call already.
let CallWaitingEnabled = getDbItem('CallWaitingEnabled', '1') === '1'
// Starts Call Recording when a call is established.
let RecordAllCalls = getDbItem('RecordAllCalls', '0') === '1'
// Starts a video call in the full screen (browser screen, not desktop)
let StartVideoFullScreen = getDbItem('StartVideoFullScreen', '1') === '1'
// Selects the ringing line if you are not on another call ()
let SelectRingingLine = getDbItem('SelectRingingLine', '1') === '1'

// Sets the max-width for the UI elements (don't set this less than 920. Set to very high number for full screen eg: 999999)
let UiMaxWidth = parseInt(getDbItem('UiMaxWidth', 1240))
// Sets the color theme for the UI dark | light | system (set by your systems dark/light settings)
let UiThemeStyle = getDbItem('UiThemeStyle', 'system')
// Put the message Stream at the top or middle can be either: top | middle
let UiMessageLayout = getDbItem('UiMessageLayout', 'middle')
// If set to true, will only call web_hook_on_config_menu
let UiCustomConfigMenu = getDbItem('UiCustomConfigMenu', '0') === '1'
// If set to true, will only call web_hook_dial_out
let UiCustomDialButton = getDbItem('UiCustomDialButton', '0') === '1'
// If set to true, will only call web_hook_sort_and_filter
let UiCustomSortAndFilterButton = getDbItem('UiCustomSortAndFilterButton', '0') === '1'
// If set to true, will only call web_hook_on_add_buddy
let UiCustomAddBuddy = getDbItem('UiCustomAddBuddy', '0') === '1'
// If set to true, will only call web_hook_on_edit_buddy({})
let UiCustomEditBuddy = getDbItem('UiCustomEditBuddy', '0') === '1'
// If set to true, will only call web_hook_on_edit_media
let UiCustomMediaSettings = getDbItem('UiCustomMediaSettings', '0') === '1'
// If set to true, will only call web_hook_on_message_action
...
// Displays the self-preview in normal or mirror view, to better present the preview.
let MirrorVideo = getDbItem('VideoOrientation', 'rotateY(180deg)')
// Suggests a frame rate to your webcam if possible.
let maxFrameRate = getDbItem('FrameRate', '')
// Suggests a video height (and therefor picture quality) to your webcam.
let videoHeight = getDbItem('VideoHeight', '')
// Specifies the maximum bandwidth (in Kb/s) for your outgoing video stream. e.g: 32 | 64 | 128 | 256 | 512 | 1024 | 2048 | -1 to disable
let MaxVideoBandwidth = parseInt(getDbItem('MaxVideoBandwidth', '2048'))
// Suggests an aspect ratio (1:1 = 1 | 4:3 = 0.75 | 16:9 = 0.5625) to your webcam.
let videoAspectRatio = getDbItem('AspectRatio', '1.33')
let NotificationsActive = getDbItem('Notifications', '0') === '1'

// The amount of rows to buffer in the Buddy Stream
let StreamBuffer = parseInt(getDbItem('StreamBuffer', 50))
// Defines the maximum amount of days worth of data (calls, recordings, messages, etc) to store locally. 0=Stores all data always. >0 Trims n days back worth of data at various events where.
const MaxDataStoreDays = parseInt(getDbItem('MaxDataStoreDays', 0))
// The image quality of the Video Poster images
let PosterJpegQuality = parseFloat(getDbItem('PosterJpegQuality', 0.6))
// The resample size (height) to re-render video that gets presented (sent). (SD = ???x360 | HD = ???x720 | FHD = ???x1080)
let VideoResampleSize = getDbItem('VideoResampleSize', 'HD')
// The size/quality of the video track in the recordings (SD = 640x360 | HD = 1280x720 | FHD = 1920x1080)
let RecordingVideoSize = getDbItem('RecordingVideoSize', 'HD')
// The Frame Per Second of the Video Track recording
let RecordingVideoFps = parseInt(getDbItem('RecordingVideoFps', 12))
// The Layout of the Recording Video Track (side-by-side | them-pnp | us-only | them-only)
let RecordingLayout = getDbItem('RecordingLayout', 'them-pnp')

// DID length from which to decide if an incoming caller is a "contact" or an "extension".
let DidLength = parseInt(getDbItem('DidLength', 6))
// Maximum length of any DID number including international dialled numbers.
let MaxDidLength = parseInt(getDbItem('MaxDidLength', 16))
// The display format for all dates. https://momentjs.com/docs/#/displaying/
let DisplayDateFormat = getDbItem('DateFormat', 'YYYY-MM-DD')
// The display format for all times. https://momentjs.com/docs/#/displaying/
let DisplayTimeFormat = getDbItem('TimeFormat', 'h:mm:ss A')
// Overrides the language selector or "automatic". Must be one of availableLang[]. If not defaults to en.
let Language = getDbItem('Language', 'auto')

// Buddy Sort and Filter
// Sorting for Buddy List display (type|extension|alphabetical|activity)
let BuddySortBy = getDbItem('BuddySortBy', 'activity')
// If the Sorting is set to type then describe the order of the types.
let SortByTypeOrder = getDbItem('SortByTypeOrder', 'e|x|c')
// Always put the Auto Delete buddies at the bottom
let BuddyAutoDeleteAtEnd = getDbItem('BuddyAutoDeleteAtEnd', '0') === '1'
// Option to not display Auto Delete Buddies (May be confusing if newly created buddies are set to auto delete.)
let HideAutoDeleteBuddies = getDbItem('HideAutoDeleteBuddies', '0') === '1'
// Controls the Extension Number display
let BuddyShowExtenNum = getDbItem('BuddyShowExtenNum', '0') === '1'

// Permission Settings
// Enables the Text Messaging
let EnableTextMessaging = getDbItem('EnableTextMessaging', '1') === '1'
// Removes the Dial icon in the profile area, users will need to add buddies in order to dial.
let DisableFreeDial = getDbItem('DisableFreeDial', '0') === '1'
// Removes the Add Someone menu item and icon from the profile area. Buddies will still be created automatically. Please also use MaxBuddies or MaxBuddyAge
let DisableBuddies = getDbItem('DisableBuddies', '0') === '1'
// Controls Transferring during a call
let EnableTransfer = getDbItem('EnableTransfer', '1') === '1'
// Controls Conference during a call
let EnableConference = getDbItem('EnableConference', '1') === '1'
// allow = user can choose | disabled = feature is disabled | enabled = feature is always on
...
// disabled = feature is disabled | enabled = feature is always on
let IntercomPolicy = getDbItem('IntercomPolicy', 'enabled')
// Controls the Account tab in Settings
let EnableAccountSettings = getDbItem('EnableAccountSettings', '1') === '1'
// Controls the Appearance tab in Settings
let EnableAppearanceSettings = getDbItem('EnableAppearanceSettings', '1') === '1'
// Controls the Notifications tab in Settings
let EnableNotificationSettings = getDbItem('EnableNotificationSettings', '1') === '1'
// Allows calling /[^\da-zA-Z\*\#\+\-\_\.\!\~\'\(\)]/g default is /[^\d\*\#\+]/g
let EnableAlphanumericDial = getDbItem('EnableAlphanumericDial', '0') === '1'
// Enables Video during a call
let EnableVideoCalling = getDbItem('EnableVideoCalling', '1') === '1'
// Enables Expressions (Emoji) glyphs when texting
let EnableTextExpressions = getDbItem('EnableTextExpressions', '1') === '1'
// Enables Dictate (speech-to-text) when texting
let EnableTextDictate = getDbItem('EnableTextDictate', '1') === '1'
// Enables a ring tone when an inbound call comes in.  (media/Ringtone_1.mp3)
let EnableRingtone = getDbItem('EnableRingtone', '1') === '1'
// Sets the Maximum number of buddies the system will accept. Older ones get deleted. (Considered when(after) adding buddies)
let MaxBuddies = parseInt(getDbItem('MaxBuddies', 999))
// Sets the Maximum age in days (by latest activity). Older ones get deleted. (Considered when(after) adding buddies)
let MaxBuddyAge = parseInt(getDbItem('MaxBuddyAge', 365))
// For automatically created buddies (inbound and outbound), should the buddy be set to AutoDelete.
const AutoDeleteDefault = getDbItem('AutoDeleteDefault', '1') === '1'

// Select the chat engine XMPP | SIMPLE
let ChatEngine = getDbItem('ChatEngine', 'SIMPLE')

// XMPP Settings
// FQDN of XMPP server HTTP service"
let XmppServer = getDbItem('XmppServer', '')
// OpenFire Default : 7443
...
// Username for auth with XMPP Server eg: 100
let profileUser = getDbItem('profileUser', null)
// XMPP Tenanting
// To create a tenant like partition in XMPP server all users and buddies will have this realm prepended to their details.
let XmppRealm = getDbItem('XmppRealm', '')
// Separates the realm from the profileUser eg: abc123-100@XmppDomain
let XmppRealmSeparator = getDbItem('XmppRealmSeparator', '-')
// TODO
let XmppChatGroupService = getDbItem('XmppChatGroupService', 'conference')

// TODO
// Enables sending of Images
const EnableSendFiles = false
// Enables sending of Images
const EnableSendImages = false
// Enables the ability to record a voice message
const EnableAudioRecording = false
// Enables the ability to record a video message
const EnableVideoRecording = false
// Enables SMS sending to the server (requires onward services)
const EnableSms = false
// Enables Fax sending to the server (requires onward services)
const EnableFax = false
// Enables Email sending to the server (requires onward services)
...
const audioBlobs = {}
let newLineNumber = 1
const telNumericRegEx = /[^\d*#+]/g
const telAlphanumericRegEx = /[^\da-zA-Z*#+\-_.!~'()]/g
...
function getAudioSrcID () {
  const id = localDB.getItem('AudioSrcId')
  return (id !== null) ? id : 'default'
}

function getAudioOutputID () {
  const id = localDB.getItem('AudioOutputId')
  return (id !== null) ? id : 'default'
}

function getVideoSrcID () {
  const id = localDB.getItem('VideoSrcId')
  return (id !== null) ? id : 'default'
}

function getRingerOutputID () {
  const id = localDB.getItem('RingOutputId')
  return (id !== null) ? id : 'default'
}
...
function GetAlternateLanguage () {
  ...
  if (userLanguage === 'en' || userLanguage.indexOf('en-') === 0) {
    // English is already loaded
    return ''
  }

  for (let l = 0; l < availableLang.length; l++) {
...
async function fetchOptions() {
  ...
  const csrfToken = csrfTokenTag === null || csrfTokenTag === undefined ? null : csrfTokenTag.content;
  const fetchOptions = {
    headers: {
  ...
  const mountingPoint = (typeof phoneOptions !== 'undefined' && phoneOptions.mountingPoint !== null && phoneOptions.mountingPoint !== undefined) ? phoneOptions.mountingPoint : ''
  const path = `${mountingPoint}/phone/configuration`
  ...
  if (response.status >= 200 && response.status < 300) {
    ...
    if (options.wssServer !== undefined) {
      wssServer = options.wssServer
    }
    ...
    if (options.VoicemailDid !== undefined) {
      VoicemailDid = options.VoicemailDid
    }
    ...
    if (options.IpInContact !== undefined) {
      IpInContact = options.IpInContact
    }
    if (options.BundlePolicy !== undefined) {
    ...
    if (options.MirrorVideo !== undefined) {
    ...
    if (options.DidLength !== undefined) {
      DidLength = options.DidLength
    }
    ...
    if (options.MaxBuddies !== undefined) {
      MaxBuddies = options.MaxBuddies
    }
    if (options.MaxBuddyAge !== undefined) {
      MaxBuddyAge = options.MaxBuddyAge
    }
    if (options.ChatEngine !== undefined) {
      ChatEngine = options.ChatEngine
    }
    if (options.XmppServer !== undefined) {
      XmppServer = options.XmppServer
    }
    ...
    if (options.XmppDomain !== undefined) {
      XmppDomain = options.XmppDomain
    }
    if (options.profileUser !== undefined) {
      profileUser = options.profileUser
    }
    if (options.XmppRealm !== undefined) {
      XmppRealm = options.XmppRealm
    }
...
function PrepareIndexDB () {
  ...
  CallQosDataOpenRequest.onupgradeneeded = function (event) {
    ...
    if (CallQosDataIndexDb.objectStoreNames.contains('CallQos') === false) {
      // Create Object Store
      const objectStore = CallQosDataIndexDb.createObjectStore('CallQos', { keyPath: 'uID' })
  ...
  CallRecordingsOpenRequest.onupgradeneeded = function (event) {
    ...
    if (CallRecordingsIndexDb.objectStoreNames.contains('Recordings') === false) {
      ...
      const objectStore = CallRecordingsIndexDb.createObjectStore('Recordings', { keyPath: 'uID' })
      objectStore.createIndex('sessionid', 'sessionid', { unique: false })
...
function UpdateUI () {
  ...
  const $leftContentTable = $('#leftContentTable')
  ...
  if (windowObj !== null) {
    const offsetTextHeight = windowObj.parent().outerHeight()
  ...
  if (alertObj !== null) {
    const width = 300
    const offsetTextHeight = alertObj.parent().outerHeight()
    if (windowWidth <= width || windowHeight <= offsetTextHeight) {
  ...
  if (confirmObj !== null) {
    const width = 300
    const offsetTextHeight = confirmObj.parent().outerHeight()
    if (windowWidth <= width || windowHeight <= offsetTextHeight) {
      if (windowWidth <= width) {
        // First apply to dialog, then set css
  ...
  if (promptObj !== null) {
    const width = 300
    const offsetTextHeight = promptObj.parent().outerHeight()
    if (windowWidth <= width || windowHeight <= offsetTextHeight) {
      if (windowWidth <= width) {
        // First apply to dialog, then set css
...
// UI Windows
// ==========
function AddSomeoneWindow (_numberStr) {
  CloseUpSettings()
  ...
  let html = '<div style="text-align:right"><button class=roundButtons onclick="ShowContacts()"><i class="fa fa-close"></i></button></div>'

  html += '<div style="border: none;" class=UiSideField>'
  ...
  buttons.push({
    ...
    action: function () {
      let type
      // Basic Validation
      const $AddSomeoneName = $('#AddSomeone_Name')
      if ($AddSomeoneName.val() === '') return
      if (type === 'extension' || type === 'xmpp') {
        if ($('#AddSomeone_Exten').val() === '') return
      }

      type = 'extension'
      const $AddSomeoneSubscribeUser = $('#AddSomeone_SubscribeUser')
      const $AddSomeoneSubscribe = $('#AddSomeone_Subscribe')
      if ($('#type_exten').is(':checked')) {
        type = 'extension'
        if ($AddSomeoneSubscribe.is(':checked') && $AddSomeoneSubscribeUser.val() === '') {
          $AddSomeoneSubscribeUser.val($('#AddSomeone_Exten').val())
        }
      ...
      let json = JSON.parse(localDB.getItem(profileUserID + '-Buddies'))
      if (json === null) {
        json = InitUserBuddies()
      }
      ...
      if (type === 'extension') {
        ...
        json.DataCollection.push(
          {
            ...
            gID: null,
            jid: null,
            DisplayName: $AddSomeoneName.val(),
            Description: $('#AddSomeone_Desc').val(),
            Email: $('#AddSomeone_Email').val(),
            MemberCount: 0,
            EnableDuringDnd: $('#AddSomeone_Dnd').is(':checked'),
            Subscribe: $AddSomeoneSubscribe.is(':checked'),
            SubscribeUser: $AddSomeoneSubscribeUser.val(),
        ...
        // Add memory object
        AddBuddy(buddyObj, false, false, $AddSomeoneSubscribe.is(':checked'), true)
      }
      if (type === 'xmpp') {
        // Add XMPP extension
        const id = uID()
        const dateNow = utcDateNow()
        let jid = $('#AddSomeone_Exten').val() + '@' + SipDomain
  ...
  $.each(buttons, function (i, obj) {
    const button = $('<button>' + obj.text + '</button>').click(obj.action)
...
function EditBuddyWindow (buddy) {
  ...
  let buddyJson = {}
  let itemId = -1
  const json = JSON.parse(localDB.getItem(profileUserID + '-Buddies'))
  ...
  let html = "<div border=0 class='UiWindowField'>"
  ...
  OpenWindow(html, lang.edit, 480, 640, false, true, lang.save, function () {
    ...
    if (buddyJson.Type === 'extension' || buddyJson.Type === 'xmpp') {
      ...
      if (buddyJson.Subscribe === true) {
        const SubscribeUser = $('#AddSomeone_SubscribeUser').val()
    ...
    $('#fileUploader').change(function () {
      const filesArray = $(this).prop('files')

      if (filesArray.length === 1) {
        const uploadId = Math.floor(Math.random() * 1000000000)
...
function SetStatusWindow () {
  ...
  let windowHtml = '<div class=UiWindowField>'
  ...
  OpenWindow(windowHtml, lang.set_status, 180, 350, false, false, 'OK', function () {
    ...
    const presenceStr = 'chat'
    const statusStr = $('#presence_text').val()
...
function InitUi () {
  ...
  let leftHTML = '<table id=leftContentTable class=leftContentTable style="height:100%; width:100%" cellspacing=0 cellpadding=0>'
  ...
  let profilePrepend = ''
  let profileVcard = getDbItem('profileVcard', null)
  if (profileVcard !== null) {
    profileVcard = JSON.parse(profileVcard)
    const displayPrefix = getDbItem('ProfileDisplayPrefix', '')
    const displayPrefixSep = getDbItem('ProfileDisplayPrefixSeparator', '-')
    if (displayPrefix !== '') {
      try {
        const vCardValue = profileVcard[displayPrefix]
...
function ShowMyProfileMenu (obj) {
  const enabledHtml = ' <i class="fa fa-check" style="float: right; line-height: 18px;"></i>'
...
function ApplyThemeColor () {
  ...
  let cssUrl = hostingPrefix + 'phone.light.css'
  let wallpaperUrl = hostingPrefix + '' + imagesDirectory + '' + wallpaperLight
  ...
  const wallpaperStyle = ".wallpaperBackground { background-image:url('" + wallpaperUrl + "') }"
...
async function CreateUserAgent () {
  const uri = SIP.UserAgent.makeURI('sip:' + SipUsername + '@' + SipDomain)
  ...
  if (RegisterExtraHeaders && RegisterExtraHeaders !== '' && RegisterExtraHeaders !== '{}') {
    try {
      const registerExtraHeaders = JSON.parse(RegisterExtraHeaders)
  ...
  if (RegisterExtraContactParams && RegisterExtraContactParams !== '' && RegisterExtraContactParams !== '{}') {
    try {
      const registerExtraContactParams = JSON.parse(RegisterExtraContactParams)
...
function ReceiveCall (session) {
  ...
  if (sipHeaders.hasOwnProperty('P-Asserted-Identity')) {
    ...
    if (rawUri.includes('<sip:')) {
      ...
      if (uriParts[1].endsWith('@' + SipDomain)) {
        const assertId = SIP.UserAgent.makeURI('sip:' + uriParts[1]) // should be sip:123@domain.com
        did = assertId.user
  ...
  let buddyObj = FindBuddyByDid(did)
  // Make new contact of its not there
  if (buddyObj === null) {
    const focusOnBuddy = (CurrentCalls === 0)
    ...
    let buddyType = (did.length > DidLength) ? 'contact' : 'extension'
    // X-Buddytype: xmpp
    ...
    let subscribeToBuddy = false
    let subscribeUser = null
    ...
    let autoDelete = AutoDeleteDefault
    // X-Autodelete: yes
  ...
  if (videoSections.length >= 1) {
    for (let m = 0; m < videoSections.length; m++) {
      if (videoSections[m].indexOf('a=mid:') > -1 && videoSections[m].indexOf('a=label:') > -1) {
        ...
        lineObj.SipSession.data.videoChannelNames.push({ mid, channel })
      }
    }
...
function onSessionReceivedMessage (lineObj, response) {
  const messageType = (response.request.headers['Content-Type'].length >= 1) ? response.request.headers['Content-Type'][0].parsed : 'Unknown'
  if (messageType.indexOf('application/x-asterisk-confbridge-event') > -1) {
    ...
    if (msgJson.type === 'ConfbridgeStart') {
      ...
      const videoContainer = $('#line-' + lineObj.LineNumber + '-remote-videos')
      if (videoContainer) {
        msgJson.channels.forEach(function (chan) {
...
function StartRemoteAudioMediaMonitoring (lineNum, session) {
  ...
  const soundMeter = new SoundMeter(session.id, lineNum)
  if (soundMeter === null) {
  ...
  soundMeter.connectToSource(remoteAudioStream, function (e) {
    ...
    soundMeter.levelsInterval = window.setInterval(function () {
      ...
      const instPercent = (soundMeter.instant / 255) * 100
      ...
      if (audioReceiver !== null) {
        audioReceiver.getStats().then(function (stats) {
          stats.forEach(function (report) {
            const theMoment = utcDateNow()
            const ReceiveBitRateChart = soundMeter.ReceiveBitRateChart
            const ReceivePacketRateChart = soundMeter.ReceivePacketRateChart
            const ReceivePacketLossChart = soundMeter.ReceivePacketLossChart
            const ReceiveJitterChart = soundMeter.ReceiveJitterChart
            const ReceiveLevelsChart = soundMeter.ReceiveLevelsChart
            const elapsedSec = Math.floor((Date.now() - soundMeter.startTime) / 1000)
            ...
            if (report.type === 'inbound-rtp') {
              ...
              // Receive Kilobits Per second
              const kbitsPerSec = (8 * (report.bytesReceived - ReceiveBitRateChart.lastValueBytesReceived)) / 1000
              ...
              // Receive Packets Per Second
              const PacketsPerSec = (report.packetsReceived - ReceivePacketRateChart.lastValuePacketReceived)
              ...
              // Receive Packet Loss
              const PacketsLost = (report.packetsLost - ReceivePacketLossChart.lastValuePacketLoss)
            ...
            if (report.type === 'track') {
              // Receive Audio Levels
              const levelPercent = (report.audioLevel * 100)
    ...
    soundMeter.networkInterval = window.setInterval(function () {
      ...
      if (audioSender !== null) {
        audioSender.getStats().then(function (stats) {
          stats.forEach(function (report) {
            const theMoment = utcDateNow()
            const SendBitRateChart = soundMeter.SendBitRateChart
            const SendPacketRateChart = soundMeter.SendPacketRateChart
            const elapsedSec = Math.floor((Date.now() - soundMeter.startTime) / 1000)
            ...
            if (report.type === 'outbound-rtp') {
              ...
              // Send Kilobits Per second
              const kbitsPerSec = (8 * (report.bytesSent - SendBitRateChart.lastValueBytesSent)) / 1000
              ...
              // Send Packets Per Second
              const PacketsPerSec = report.packetsSent - SendPacketRateChart.lastValuePacketSent
...
class SoundMeter {
  ...
  _start () {
    ...
    this.captureInterval = window.setInterval(function () {
      ...
      self.instant = 0
      for (let d = 0; d < self.dataArray.length; d++) {
  ...
  stop () {
    ...
    // Save to IndexDb
    const lineObj = FindLineByNumber(this.lineNum)
    const QosData = {
...
function MeterSettingsOutput (audioStream, objectId, direction, interval) {
  const soundMeter = new SoundMeter(null, null)
  soundMeter.startTime = Date.now()
...
function SaveQosData (QosData, sessionId, buddy) {
  if (CallQosDataIndexDb !== null) {
    ...
    // Commit Transaction
    const transaction = CallQosDataIndexDb.transaction(['CallQos'], 'readwrite')
    const objectStoreAdd = transaction.objectStore('CallQos').add(data)
...
function DisplayQosData (sessionId) {
  if (CallQosDataIndexDb !== null) {
    const transaction = CallQosDataIndexDb.transaction(['CallQos'])
    const objectStoreGet = transaction.objectStore('CallQos').index('sessionid').getAll(sessionId)
    ...
    objectStoreGet.onsuccess = function (event) {
      ...
      const ReceiveBitRateChart = new Chart($('#cdr-AudioReceiveBitRate'), {
        type: 'line',
        data: {
          labels: labelSet,
          datasets: [{
      ...
      const ReceivePacketRateChart = new Chart($('#cdr-AudioReceivePacketRate'), {
        type: 'line',
        data: {
          labels: labelSet,
          datasets: [{
      ...
      const AudioReceivePacketLossChart = new Chart($('#cdr-AudioReceivePacketLoss'), {
        type: 'line',
        data: {
          labels: labelSet,
          datasets: [{
      ...
      const AudioReceiveJitterChart = new Chart($('#cdr-AudioReceiveJitter'), {
        type: 'line',
        data: {
          labels: labelSet,
          datasets: [{
      ...
      const AudioReceiveLevelsChart = new Chart($('#cdr-AudioReceiveLevels'), {
        type: 'line',
        data: {
          labels: labelSet,
          datasets: [{
      ...
      const SendPacketRateChart = new Chart($('#cdr-AudioSendPacketRate'), {
        type: 'line',
        data: {
          labels: labelSet,
          datasets: [{
      ...
      const AudioSendBitRateChart = new Chart($('#cdr-AudioSendBitRate'), {
        type: 'line',
        data: {
          labels: labelSet,
          datasets: [{
  ...
  if (CallQosDataIndexDb !== null) {
    ...
    $.each(stream.DataCollection, function (i, item) {
      if (item.ItemType === 'CDR' && item.SessionId && item.SessionId !== '') {
        ...
        const objectStore = CallQosDataIndexDb.transaction(['CallQos'], 'readwrite').objectStore('CallQos')
        const objectStoreGet = objectStore.index('sessionid').getAll(item.SessionId)
        objectStoreGet.onerror = function (event) {
          console.error('IndexDB Get Error:', event)
        }
        objectStoreGet.onsuccess = function (event) {
...
function SelfSubscribe () {
  ...
  const targetURI = SIP.UserAgent.makeURI('sip:' + SipUsername + '@' + SipDomain)
...
function SubscribeVoicemail () {
  ...
  const vmOptions = { expires: SubscribeVoicemailExpires }
  const targetURI = SIP.UserAgent.makeURI('sip:' + SipUsername + '@' + SipDomain)
...
function SubscribeBuddy (buddyObj) {
  ...
  if ((buddyObj.type === 'extension' || buddyObj.type === 'xmpp') && buddyObj.EnableSubscribe === true && buddyObj.SubscribeUser !== '') {
    const targetURI = SIP.UserAgent.makeURI('sip:' + buddyObj.SubscribeUser + '@' + SipDomain)

    const options = {
      expires: SubscribeBuddyExpires,
      extraHeaders: ['Accept: ' + SubscribeBuddyAccept]
    }
    const blfSubscribe = new SIP.Subscriber(userAgent, targetURI, SubscribeBuddyEvent, options)
    blfSubscribe.data = {}
...
function UnsubscribeBuddy (buddyObj) {
  ...
  if (buddyObj.type === 'extension' || buddyObj.type === 'xmpp') {
    if (userAgent && userAgent.BlfSubs && userAgent.BlfSubs.length > 0) {
      for (let blf = 0; blf < userAgent.BlfSubs.length; blf++) {
        const blfSubscribe = userAgent.BlfSubs[blf]
...
function VoicemailNotify (notification) {
  ...
  if (notification.request.body.indexOf('Messages-Waiting:') > -1) {
    ...
    const messagesWaiting = (notification.request.body.indexOf('Messages-Waiting: yes') > -1)
    ...
    if (messagesWaiting) {
      ...
      const lines = notification.request.body.split('\r\n')
      for (let l = 0; l < lines.length; l++) {
        if (lines[l].indexOf('Voice-Message: ') > -1) {
          const value = lines[l].replace('Voice-Message: ', '') // 1/0 (0/0)
          if (value.indexOf(' (') > -1) {
            // With Ugent options
      ...
      if (newVoiceMessages > userAgent.lastVoicemailCount) {
        ...
        if ('Notification' in window) {
          if (Notification.permission === 'granted') {
            ...
            const vmNotification = new Notification(lang.new_voice_mail, noticeOptions)
...
closed: In the context of INSTANT MESSAGES, this value means that
    ...
    const tuples = xml.find('presence').find('tuple')
    if (tuples) {
    ...
    *           <local>
    ...
    const version = xml.find('dialog-info').attr('version') // 1|2|etc
    const DialogState = xml.find('dialog-info').attr('state') // full|partial
    const extId = xml.find('dialog-info').find('dialog').attr('id') // qoe2vr886cbn1ccj3h.0

    const state = xml.find('dialog-info').find('dialog').find('state').text()
  ...
  const buddyObj = FindBuddyByObservedUser(buddy)
  if (buddyObj === null) {
...
function InitialiseStream (buddy) {
  const template = { TotalRows: 0, DataCollection: [] }
...
function SendChatMessage (buddy) {
  ...
  let message = $('#contact-' + buddy + '-ChatMessage').val()
  ...
  let currentStream = JSON.parse(localDB.getItem(buddy + '-stream'))
  ...
  if (buddyObj.type === 'extension') {
    const chatBuddy = SIP.UserAgent.makeURI('sip:' + buddyObj.ExtNo.replace(/#/g, '%23') + '@' + SipDomain)
    console.log('MESSAGE: ' + chatBuddy + ' (extension)')

    const MessagerMessageOptions = {
      requestDelegate: {
    ...
    const messageObj = new SIP.Messager(userAgent, chatBuddy, message, 'text/plain')
...
function MarkMessageSent (buddyObj, messageId, refresh) {
  const currentStream = JSON.parse(localDB.getItem(buddyObj.identity + '-stream'))
...
function ReceiveOutOfDialogMessage (message) {
  const callerID = message.request.from.displayName
  const did = message.request.from.uri.normal.user

  // Out of dialog Message Receiver
  const messageType = (message.request.headers['Content-Type'].length >= 1) ? message.request.headers['Content-Type'][0].parsed : 'Unknown'
  // Text Messages
  if (messageType.indexOf('text/plain') > -1) {
    ...
    const originalMessage = message.request.body
    const messageId = uID()
...
function AddMessageToStream (buddyObj, messageId, type, message, DateTime) {
  let currentStream = JSON.parse(localDB.getItem(buddyObj.identity + '-stream'))
...
function ActivateStream (buddyObj, message) {
  ...
  if (!streamVisible) {
    ...
    if ('Notification' in window) {
      if (Notification.permission === 'granted') {
        const imageUrl = getPicture(buddyObj.identity)
        const noticeOptions = { body: message.substring(0, 250), icon: imageUrl }
        const inComingChatNotification = new Notification(lang.message_from + ' : ' + buddyObj.CallerIDName, noticeOptions)
    ...
    const ringer = new Audio(audioBlobs.Alert.blob)
    ringer.preload = 'auto'
    ringer.loop = false
    ringer.oncanplaythrough = function (e) {
      if (typeof ringer.sinkId !== 'undefined' && getRingerOutputID() !== 'default') {
...
function AddCallMessage (buddy, session) {
  ...
  const CallEnd = moment.utc() // Take Now as the Hangup Time
  let callDuration = 0
  let totalDuration = 0
  let ringTime = 0

  const CallStart = moment.utc(session.data.callstart.replace(' UTC', '')) // Actual start (both inbound and outbound)
  let CallAnswer = null // On Accept when inbound, Remote Side when Outbound
  ...
  const callDirection = session.data.calldirection
  const withVideo = session.data.withvideo
  const sessionId = session.id
  const hangupBy = session.data.terminateby
...
function SendImageDataMessage (buddy, ImgDataUrl) {
  ...
  const DateTime = moment.utc().format('YYYY-MM-DD HH:mm:ss UTC')
  const formattedMessage = '<IMG class=previewImage onClick="PreviewImage(this)" src="' + ImgDataUrl + '">'
...
function updateLineScroll (lineNum) {
  ...
  const element = $('#line-' + lineNum + '-CallDetails').get(0)
  if (element) element.scrollTop = element.scrollHeight
}

function updateScroll (buddy) {
  const history = $('#contact-' + buddy + '-ChatHistory')
  try {
...
function VideoCall (lineObj, dialledNumber, extraHeaders) {
  ...
  const targetURI = SIP.UserAgent.makeURI('sip:' + dialledNumber.replace(/#/g, '%23') + '@' + SipDomain)
...
function AudioCallMenu (buddy, obj) {
  ...
  if (items.length === 1) {
    ...
    const menu = {
      selectEvent: function (event, ui) {
        const number = ui.item.attr('value')
        HidePopup()
        if (number !== null) {
...
function StartRecording (lineNum) {
  ...
  if (session.data.withvideo) {
    ...
    session.data.recordingRedrawInterval = window.setInterval(function () {
      // Video Source
      const pnpVideo = $('#line-' + lineObj.LineNumber + '-localVideo').get(0)

      let mainVideo = null
      const validVideos = []
      const talkingVideos = []
      const videoContainer = $('#line-' + lineObj.LineNumber + '-remote-videos')
      const potentialVideos = videoContainer.find('video').length
      ...
      const offsetY = (videoHeight < recordingHeight) ? (recordingHeight - videoHeight) / 2 : 0
...
function SaveCallRecording (blob, id, buddy, sessionid) {
  if (CallRecordingsIndexDb !== null) {
    ...
    // Commit Transaction
    const transaction = CallRecordingsIndexDb.transaction(['Recordings'], 'readwrite')
    const objectStoreAdd = transaction.objectStore('Recordings').add(data)
    objectStoreAdd.onsuccess = function (event) {
...
function PlayAudioCallRecording (obj, cdrId, uID) {
  ...
  if (CallRecordingsIndexDb !== null) {
    const transaction = CallRecordingsIndexDb.transaction(['Recordings'])
    const objectStoreGet = transaction.objectStore('Recordings').get(uID)
    objectStoreGet.onerror = function (event) {
      console.error('IndexDB Get Error:', event)
    }
    objectStoreGet.onsuccess = function (event) {
          ...
          window.setTimeout(function () {
            ...
            canvas.toBlob(function (blob) {
              ...
              reader.onloadend = function () {
                const Poster = { width: videoWidth, height: videoHeight, posterBase64: reader.result }
                console.log('Capturing Video Poster...')

                // Update DB
                const currentStream = JSON.parse(localDB.getItem(buddy + '-stream'))
                if (currentStream !== null || currentStream.DataCollection !== null) {
                  $.each(currentStream.DataCollection, function (i, item) {
    ...
    if (display) {
      ...
      if (buddyObj.ExtNo !== '') {
        items.push({
          ...
          text: lang.extension + ' (' + buddyObj.presence + '): ' + buddyObj.ExtNo,
          value: buddyObj.ExtNo
        })
      }
...
function BlindTransfer (lineNum) {
  let dstNo = $('#line-' + lineNum + '-txt-FindTransferBuddy').val()
  ...
  const referTo = SIP.UserAgent.makeURI('sip:' + dstNo.replace(/#/g, '%23') + '@' + SipDomain)
...
function AttendedTransfer (lineNum) {
  ...
  const newCallStatus = $('#line-' + lineNum + '-transfer-status')
  ...
  const targetURI = SIP.UserAgent.makeURI('sip:' + dstNo.replace(/#/g, '%23') + '@' + SipDomain)
  const newSession = new SIP.Inviter(userAgent, targetURI, spdOptions)
  newSession.data = {}
  newSession.delegate = {
    onBye: function (sip) {
      ...
      if (sdh) {
        if (sdh.peerConnection) {
          sdh.peerConnection.ontrack = function (event) {
            ...
            const remoteAudio = $('#line-' + lineNum + '-transfer-remoteAudio').get(0)
  ...
  const inviterOptions = {
    requestDelegate: {
      ...
      onProgress: function (sip) {
        ...
        const CancelAttendedTransferBtn = $('#line-' + lineNum + '-btn-cancel-attended-transfer')
      ...
      onAccept: function (sip) {
        ...
        const CompleteTransferBtn = $('#line-' + lineNum + '-btn-complete-attended-transfer')
        ...
        const TerminateAttendedTransferBtn = $('#line-' + lineNum + '-btn-terminate-attended-transfer')
...
function ConferenceDial (lineNum) {
  let dstNo = $('#line-' + lineNum + '-txt-FindConferenceBuddy').val()
  if (EnableAlphanumericDial) {
    dstNo = dstNo.replace(telAlphanumericRegEx, '').substring(0, MaxDidLength)
  } else {
    dstNo = dstNo.replace(telNumericRegEx, '').substring(0, MaxDidLength)
  ...
  const newCallStatus = $('#line-' + lineNum + '-conference-status')
  newCallStatus.html(lang.connecting)
  newCallStatus.show()
  ...
  const confCallId = session.data.confcalls.length - 1
      ...
      if (sdh) {
        if (sdh.peerConnection) {
          sdh.peerConnection.ontrack = function (event) {
            ...
            const remoteAudio = $('#line-' + lineNum + '-conference-remoteAudio').get(0)
            remoteAudio.srcObject = remoteStream
            remoteAudio.onloadedmetadata = function (e) {
              if (typeof remoteAudio.sinkId !== 'undefined') {
                remoteAudio.setSinkId(session.data.AudioOutputDevice).then(function () {
  ...
  const inviterOptions = {
    requestDelegate: {
      ...
      onProgress: function (sip) {
        ...
        const CancelConferenceDialBtn = $('#line-' + lineNum + '-btn-cancel-conference-dial')
      ...
      onAccept: function (sip) {
        ...
        // Join Call
        const JoinCallBtn = $('#line-' + lineNum + '-btn-join-conference-call')
        ...
        JoinCallBtn.on('click', function () {
          ...
          const outputStreamForSession = new MediaStream()
          const outputStreamForConfSession = new MediaStream()

          const pc = session.sessionDescriptionHandler.peerConnection
          const confPc = session.data.childsession.sessionDescriptionHandler.peerConnection
          ...
          pc.getSenders().forEach(function (RTCRtpSender) {
            if (RTCRtpSender.track && RTCRtpSender.track.kind === 'audio') {
              ...
              const mixedAudioTrack = MixAudioStreams(outputStreamForSession).getAudioTracks()[0]
          ...
          confPc.getSenders().forEach(function (RTCRtpSender) {
            if (RTCRtpSender.track && RTCRtpSender.track.kind === 'audio') {
              ...
              const mixedAudioTrackForConf = MixAudioStreams(outputStreamForConfSession).getAudioTracks()[0]
        ...
        // End Call
        const TerminateConfCallBtn = $('#line-' + lineNum + '-btn-terminate-conference-call')
...
function switchVideoSource (lineNum, srcId) {
  ...
  navigator.mediaDevices.getUserMedia(constraints).then(function (newStream) {
    const newMediaTrack = newStream.getVideoTracks()[0]
  ...
  const localVideo = $('#line-' + lineNum + '-localVideo').get(0)
...
function SendCanvas (lineNum) {
  ...
  const canvasMediaStream = $('#line-' + lineNum + '-scratchpad').get(0).captureStream(25)
  const canvasMediaTrack = canvasMediaStream.getVideoTracks()[0]
...
function SendVideo (lineNum, src) {
  ...
  // Create Video Object
  const newVideo = $('#line-' + lineNum + '-sharevideo')
  ...
  newVideo.on('loadedmetadata', function () {
    ...
    const videoObj = newVideo.get(0)
    const resampleCanvas = $('<canvas/>').get(0)
    ...
    const resampleVideoMediaStream = resampleCanvas.captureStream(25)

    // Get the Tracks
    const videoMediaTrack = resampleVideoMediaStream.getVideoTracks()[0]
    const audioTrackFromVideo = (videoMediaStream !== null) ? videoMediaStream.getAudioTracks()[0] : null
    ...
    pc.getSenders().forEach(function (RTCRtpSender) {
      ...
      if (RTCRtpSender.track && RTCRtpSender.track.kind === 'audio') {
        ...
        const mixedAudioTrack = MixAudioStreams(mixedAudioStream).getAudioTracks()[0]
        mixedAudioTrack.IsMixedTrack = true
...
function KeyPress (num) {
  const currVal = $('#dialText').val()
  const textElObj = $('#dialText').get(0)
  const ss = textElObj.selectionStart
  const se = textElObj.selectionEnd
  const ln = currVal.length
...
function CloseUpSettings () {
  ...
  try {
    const localVideo = $('#local-video-preview').get(0)
...
function DialByLine (type, buddy, numToDial, CallerID, extraHeaders) {
  ...
  let numDial = (numToDial) || $('#dialText').val()
  if (EnableAlphanumericDial) {
  ...
  let buddyObj = (buddy) ? FindBuddyByIdentity(buddy) : FindBuddyByDid(numDial)
  if (buddyObj === null) {
    let buddyType = (numDial.length > DidLength) ? 'contact' : 'extension'
    ...
    buddyObj = MakeBuddy(buddyType, true, false, false, (CallerID) || numDial, numDial, null, false, null, AutoDeleteDefault, false)
  }
...
function SelectLine (lineNum) {
  ...
  // Update Lines List
  for (var l = 0; l < Lines.length; l++) {
    let classStr = (Lines[l].LineNumber === lineObj.LineNumber) ? 'buddySelected' : 'buddy'
...
function AddLineHtml (lineObj, direction) {
  const avatar = getPicture(lineObj.BuddyObj.identity)

  let html = '<table id="line-ui-' + lineObj.LineNumber + '" class=stream cellspacing=0 cellpadding=0>'
...
function RemoveLine (lineObj) {
  ...
  const earlyReject = lineObj.SipSession.data.earlyReject
  for (let l = 0; l < Lines.length; l++) {
...
function RefreshLineActivity (lineNum) {
  ...
  let ringTime = 0
  let CallStart = moment.utc(session.data.callstart.replace(' UTC', ''))
  let CallAnswer = null
  if (session.data.startTime) {
  ...
  const withVideo = (session.data.withvideo) ? '(' + lang.with_video + ')' : ''
  const startCallMessage = (session.data.calldirection === 'inbound') ? lang.you_received_a_call_from + ' ' + srcCallerID + ' ' + withVideo : lang.you_made_a_call_to + ' ' + dstCallerID + ' ' + withVideo
  callDetails.push({
    Message: startCallMessage,
    TimeStr: CallStart
  })
  if (CallAnswer) {
    const answerCallMessage = (session.data.calldirection === 'inbound') ? lang.you_answered_after + ' ' + ringTime + ' ' + lang.seconds_plural : lang.they_answered_after + ' ' + ringTime + ' ' + lang.seconds_plural
    callDetails.push({
  ...
  const Transfers = (session.data.transfer) ? session.data.transfer : []
  $.each(Transfers, function (item, transfer) {
    let msg = (transfer.type === 'Blind') ? lang.you_started_a_blind_transfer_to + ' ' + transfer.to + '. ' : lang.you_started_an_attended_transfer_to + ' ' + transfer.to + '. '
  ...
  const Mutes = (session.data.mute) ? session.data.mute : []
  ...
  const Holds = (session.data.hold) ? session.data.hold : []
  ...
  const ConfbridgeEvents = (session.data.ConfbridgeEvents) ? session.data.ConfbridgeEvents : []
  ...
  const Recordings = (session.data.recordings) ? session.data.recordings : []
  $.each(Recordings, function (item, recording) {
    let msg = lang.call_is_being_recorded
  ...
  const ConfCalls = (session.data.confcalls) ? session.data.confcalls : []
  $.each(ConfCalls, function (item, confCall) {
    let msg = lang.you_started_a_conference_call_to + ' ' + confCall.to + '. '
  ...
  callDetails.sort(function (a, b) {
    const aMo = moment.utc(a.TimeStr.replace(' UTC', ''))
    const bMo = moment.utc(b.TimeStr.replace(' UTC', ''))
  ...
  $.each(callDetails, function (item, detail) {
    const Time = moment.utc(detail.TimeStr.replace(' UTC', '')).local().format(DisplayTimeFormat)
    let messageString = '<table class=timelineMessage cellspacing=0 cellpadding=0><tr>'
...
var Buddy = function (type, identity, CallerIDName, ExtNo, MobileNumber, ContactNumber1, ContactNumber2, lastActivity, desc, Email, jid, dnd, subscribe, subscription, autoDelete, pinned) {
  ...
  this.SubscribeUser = (subscription) || ExtNo
...
function CleanupBuddies () {
  if (MaxBuddyAge > 1 || MaxBuddies > 1) {
    ...
    Buddies.sort(function (a, b) {
      const aMo = moment.utc(a.lastActivity.replace(' UTC', ''))
      const bMo = moment.utc(b.lastActivity.replace(' UTC', ''))
      if (aMo.isSameOrAfter(bMo, 'second')) {
        return -1
      } else return 1
      return 0
    ...
    if (MaxBuddyAge > 1) {
      const expiredDate = moment.utc().subtract(MaxBuddyAge, 'days')
      ...
      for (var b = Buddies.length - 1; b >= 0; b--) {
        const lastActivity = moment.utc(Buddies[b].lastActivity.replace(' UTC', ''))
...
function PopulateBuddyList () {
  ...
  $.each(json.DataCollection, function (i, item) {
    item.AutoDelete = (item.AutoDelete === true)
...
function UpdateBuddyList () {
  ...
  for (var b = 0; b < Buddies.length; b++) {
    ...
    const lastActivity = moment.utc(buddyObj.lastActivity.replace(' UTC', ''))
    let displayDateTime = ''
    ...
    if (buddyObj.type === 'extension') {
      ...
      friendlyState = friendlyState.replace(/[<>"'\r\n&]/g, function (chr) {
        const table = { '<': 'lt', '>': 'gt', '"': 'quot', '\'': 'apos', '&': 'amp', '\r': '#10', '\n': '#13' }
        return '&' + table[chr] + ';'
      })
...
function AddBuddyMessageStream (buddyObj) {
  ...
  // Presence
  if (buddyObj.type === 'extension') {
    let friendlyState = buddyObj.presence
    if (friendlyState === 'Unknown') friendlyState = lang.state_unknown
    if (friendlyState === 'Not online') friendlyState = lang.state_not_online
    if (friendlyState === 'Ready') friendlyState = lang.state_ready
    if (friendlyState === 'On the phone') friendlyState = lang.state_on_the_phone
  ...
  let html = '<table id="stream-' + buddyObj.identity + '" class=stream cellspacing=0 cellpadding=0>'
...
function RemoveBuddyMessageStream (buddyObj, days) {
  ...
  let stream = JSON.parse(localDB.getItem(buddyObj.identity + '-stream'))
  if (days && days > 0) {
    if (stream && stream.DataCollection && stream.DataCollection.length >= 1) {
      ...
      trimmedStream.DataCollection = stream.DataCollection.filter(function (item) {
        // Apply Date Filter
        const itemDate = moment.utc(item.ItemDate.replace(' UTC', ''))
        const expiredDate = moment().utc().subtract(days, 'days')
        // Condition
    ...
    $.each(stream.DataCollection, function (i, item) {
      if (item.ItemType === 'CDR' && item.Recordings && item.Recordings.length) {
        $.each(item.Recordings, function (i, recording) {
          ...
          const objectStore = CallRecordingsIndexDb.transaction(['Recordings'], 'readwrite').objectStore('Recordings')
          try {
            const deleteRequest = objectStore.delete(recording.uID)
...
function ToggleExtraButtons (lineNum, normal, expanded) {
  const extraButtons = $('#contact-' + lineNum + '-extra-buttons')
...
function SortBuddies () {
  ...
  if (BuddySortBy === 'extension') {
    Buddies.sort(function (a, b) {
      const aSortBy = (a.type === 'extension' || a.type === 'xmpp') ? a.ExtNo : a.ContactNumber1
      const bSortBy = (b.type === 'extension' || b.type === 'xmpp') ? b.ExtNo : a.ContactNumber1
      const aMo = moment.utc(a.lastActivity.replace(' UTC', ''))
      const bMo = moment.utc(b.lastActivity.replace(' UTC', ''))
...
function SelectBuddy (buddy) {
  ...
  const displayName = (BuddyShowExtenNum === true && (buddyObj.type === 'extension' || buddyObj.type === 'xmpp')) ? ' ' + buddyObj.ExtNo + ' - ' + buddyObj.CallerIDName : buddyObj.CallerIDName
...
function RefreshStream (buddyObj, filter) {
  ...
  const json = JSON.parse(localDB.getItem(buddyObj.identity + '-stream'))
  if (json === null || json.DataCollection === null) return

  // Sort DataCollection (Newest items first)
  json.DataCollection.sort(function (a, b) {
    const aMo = moment.utc(a.ItemDate.replace(' UTC', ''))
    const bMo = moment.utc(b.ItemDate.replace(' UTC', ''))
    if (aMo.isSameOrAfter(bMo, 'second')) {
      return -1
    } else return 1
    return 0
  ...
  if (filter && filter !== '') {
    ...
    json.DataCollection = json.DataCollection.filter(function (item) {
      if (filter.indexOf('date: ') !== -1) {
        // Apply Date Filter
        const dateFilter = getFilter(filter, 'date')
      ...
      if (item.ItemType === 'MSG') {
        ...
        if (item.Tags && item.Tags.length > 1) {
          const tagFilter = getFilter(filter, 'tag')
          if (tagFilter !== '') {
  ...
  $.each(json.DataCollection, function (i, item) {
    const IsToday = moment.utc(item.ItemDate.replace(' UTC', '')).isSame(moment.utc(), 'day')
    let DateTime = moment.utc(item.ItemDate.replace(' UTC', '')).local().calendar(null, { sameElse: DisplayDateFormat })
    ...
    if (item.ItemType === 'MSG') {
      ...
      let deliveryStatus = '<i class="fa fa-question-circle-o SendingMessage"></i>'
      ...
      const longMessage = (formattedMessage.length > 1000)
      ...
      const iconColor = (item.Billsec > 0) ? 'green' : 'red'
      var formattedMessage = ''

      // Flagged
      let flag = '<span id=cdr-flagged-' + item.CdrId + ' style="' + ((item.Flagged) ? '' : 'display:none') + '">'
      ...
      let CallTags = '<ul id=cdr-tags-' + item.CdrId + ' class=tags style="' + ((item.Tags && item.Tags.length > 0) ? '' : 'display:none') + '">'
      ...
      // Call Type
      const callIcon = (item.WithVideo) ? 'fa-video-camera' : 'fa-phone'
      formattedMessage += '<i class="fa ' + callIcon + '" style="color:' + iconColor + '"></i>'
      const audioVideo = (item.WithVideo) ? lang.a_video_call : lang.an_audio_call
      ...
      if (item.Recordings && item.Recordings.length >= 1) {
        $.each(item.Recordings, function (i, recording) {
          if (recording.uID) {
            const StartTime = moment.utc(recording.startTime.replace(' UTC', '')).local()
            const StopTime = moment.utc(recording.stopTime.replace(' UTC', '')).local()
            const recordingDuration = moment.duration(StopTime.diff(StartTime))
            recordingsHtml += '<div class=callRecording>'
            if (item.WithVideo) {
              if (recording.Poster) {
                const posterWidth = recording.Poster.width
                const posterHeight = recording.Poster.height
                const posterImage = recording.Poster.posterBase64
...
function RedrawStage (lineNum, videoChanged) {
  const stage = $('#line-' + lineNum + '-VideoCall')
  const container = $('#line-' + lineNum + '-stage-container')
  const previewContainer = $('#line-' + lineNum + '-preview-container')
  const videoContainer = $('#line-' + lineNum + '-remote-videos')
  ...
  let videoCount = 0
  videoContainer.find('video').each(function (i, video) {
    const thisRemoteVideoStream = video.srcObject
    const videoTrack = thisRemoteVideoStream.getVideoTracks()[0]
    const videoTrackSettings = videoTrack.getSettings()
    const srcVideoWidth = (videoTrackSettings.width) ? videoTrackSettings.width : video.videoWidth
    const srcVideoHeight = (videoTrackSettings.height) ? videoTrackSettings.height : video.videoHeight
  ...
  const stageWidth = videoContainer.outerWidth() - (Margin * 2)
  const stageHeight = videoContainer.outerHeight() - (Margin * 2)
  var previewWidth = previewContainer.outerWidth()
  let maxWidth = 0
  let i = 1
  while (i < 5000) {
    const w = StageArea(i, videoCount, stageWidth, stageHeight, Margin, videoRatio)
    if (w === false) {
      maxWidth = i - 1
      break
    }
  ...
  videoContainer.find('video').each(function (i, video) {
    ...
    const videoHeight = maxWidth * videoRatio
...
function ShowMessageMenu (obj, typeStr, cdrId, buddy) {
  const items = []
  if (typeStr === 'CDR') {
    const TagState = $('#cdr-flagged-' + cdrId).is(':visible')
    const TagText = (TagState) ? lang.clear_flag : lang.flag_call
    ...
    selectEvent: function (event, ui) {
      ...
      if (id !== null) {
        ...
        if (id === 1) {
          ...
          const callDetails = []
          let html = '<div class="UiWindowField">'
          ...
          const CallDate = moment.utc(cdr.ItemDate.replace(' UTC', '')).local().format(DisplayDateFormat + ' ' + DisplayTimeFormat)
          const CallAnswer = (cdr.CallAnswer) ? moment.utc(cdr.CallAnswer.replace(' UTC', '')).local().format(DisplayDateFormat + ' ' + DisplayTimeFormat) : null
          const ringTime = (cdr.RingTime) ? cdr.RingTime : 0
          const CallEnd = moment.utc(cdr.CallEnd.replace(' UTC', '')).local().format(DisplayDateFormat + ' ' + DisplayTimeFormat)
          ...
          const withVideo = (cdr.WithVideo) ? '(' + lang.with_video + ')' : ''
          const startCallMessage = (cdr.CallDirection === 'inbound') ? lang.you_received_a_call_from + ' ' + srcCallerID + ' ' + withVideo : lang.you_made_a_call_to + ' ' + dstCallerID + ' ' + withVideo
          callDetails.push({
            Message: startCallMessage,
            TimeStr: cdr.ItemDate
          })
          if (CallAnswer) {
            const answerCallMessage = (cdr.CallDirection === 'inbound') ? lang.you_answered_after + ' ' + ringTime + ' ' + lang.seconds_plural : lang.they_answered_after + ' ' + ringTime + ' ' + lang.seconds_plural
            callDetails.push({
              Message: answerCallMessage,
          ...
          OpenWindow(html, lang.call_detail_record, 480, 640, false, true, null, null, lang.cancel, function () {
            ...
            $.each(cdr.Recordings, function (r, recording) {
              ...
              const downloadURL = $('#download-' + recording.uID)
              ...
              if (CallRecordingsIndexDb !== null) {
                const transaction = CallRecordingsIndexDb.transaction(['Recordings'])
                const objectStoreGet = transaction.objectStore('Recordings').get(recording.uID)
                objectStoreGet.onerror = function (event) {
                  console.error('IndexDB Get Error:', event)
                }
                objectStoreGet.onsuccess = function (event) {
                  const mediaBlobUrl = window.URL.createObjectURL(event.target.result.mediaBlob)
        ...
        if (id === 4) {
          const currentText = $('#cdr-comment-' + cdrId).text()
          $('#cdr-comment-' + cdrId).empty()

          const textboxObj = $('<input maxlength=500 type=text>').appendTo('#cdr-comment-' + cdrId)
        ...
        if (id === 12) {
          var msgtext = $('#msg-text-' + cdrId).text()
          msgtext = '"' + msgtext + '"'
          const textarea = $('#contact-' + buddy + '-ChatMessage')
...
function ShowEmojiBar (buddy) {
  const messageContainer = $('#contact-' + buddy + '-emoji-menu')
  const textarea = $('#contact-' + buddy + '-ChatMessage')
  ...
  $.each(emojis, function (i, e) {
    ...
    emoji.on('click', function () {
      const i = textarea.prop('selectionStart')
...
function ShowDictate (buddy) {
  ...
  const messageContainer = $('#contact-' + buddy + '-dictate-message')
  const textarea = $('#contact-' + buddy + '-ChatMessage')
...
function ShowMyProfile () {
  ...
  let AccountHtml = '<div id=Configure_Extension_Html style="display:none">'
  ...
  let AudioVideoHtml = '<div id=Audio_Video_Html style="display:none">'
  ...
  let AppearanceHtml = '<div id=Appearance_Html style="display:none">'
  ...
  let NotificationsHtml = '<div id=Notifications_Html style="display:none">'
  ...
  buttons.push({
    text: lang.save,
    action: function () {
      const chatEng = ($('#chat_type_sip').is(':checked')) ? 'SIMPLE' : 'XMPP'
  ...
  window.setTimeout(function () {
    ...
    // Audio Preview
    const playButton = $('#preview_output_play')
    ...
    playButton.click(function () {
      ...
      try {
        const tracks = window.SettingsOutputStream.getTracks()
        tracks.forEach(function (track) {
          track.stop()
        })
      } catch (e) {
      ...
      try {
        const soundMeter = window.SettingsOutputStreamMeter
        soundMeter.stop()
      } catch (e) {
      }
      window.SettingsOutputStreamMeter = null
      ...
      const audioObj = new Audio(audioBlobs.speech_orig.blob)
    ...
    const playRingButton = $('#preview_ringer_play')
    ...
    playRingButton.click(function () {
      ...
      try {
        const tracks = window.SettingsRingerStream.getTracks()
        tracks.forEach(function (track) {
          track.stop()
        })
      } catch (e) {
      ...
      try {
        const soundMeter = window.SettingsRingerStreamMeter
        soundMeter.stop()
      } catch (e) {
      }
      window.SettingsRingerStreamMeter = null
      ...
      // Load Sample
      console.log('Audio:', audioBlobs.Ringtone.url)
      const audioObj = new Audio(audioBlobs.Ringtone.blob)
      audioObj.preload = 'auto'
      audioObj.onplay = function () {
        let outputStream = new MediaStream()
        if (typeof audioObj.captureStream !== 'undefined') {
    ...
    selectAudioScr.change(function () {
      ...
      const audioObj = window.SettingsOutputAudio
      if (audioObj !== null) {
        if (typeof audioObj.sinkId !== 'undefined') {
    ...
    // Microphone
    const selectMicScr = $('#microphoneSrc')
    ...
    selectMicScr.change(function () {
      ...
      try {
        const tracks = window.SettingsMicrophoneStream.getTracks()
        tracks.forEach(function (track) {
          track.stop()
        })
      ...
      const localMicrophoneStream = new MediaStream()
    ...
    if (EnableVideoCalling === true) {
      ...
      // Orientation
      const OriteationSel = $('input[name=Settings_Orientation]')
      ...
      // Frame Rate
      const frameRateSel = $('input[name=Settings_FrameRate]')
      ...
      // Quality
      const QualitySel = $('input[name=Settings_Quality]')
      ...
      // Aspect Ratio
      const AspectRatioSel = $('input[name=Settings_AspectRatio]')
    ...
    if (navigator.mediaDevices) {
      navigator.mediaDevices.enumerateDevices().then(function (deviceInfos) {
        const savedVideoDevice = getVideoSrcID()
        let videoDeviceFound = false

        const savedAudioDevice = getAudioSrcID()
        ...
        navigator.mediaDevices.getUserMedia(contraints).then(function (mediaStream) {
          ...
          for (let i = 0; i < deviceInfos.length; ++i) {
            ...
            let DisplayName = deviceInfo.label
    ...
    // Notifications
    if (EnableNotificationSettings) {
      const NotificationsCheck = $('#Settings_Notifications')
...
function ToggleAutoAnswer () {
  ...
  AutoAnswerEnabled = AutoAnswerEnabled !== true
...
function ToggleCallWaiting () {
  ...
  CallWaitingEnabled = CallWaitingEnabled !== true
...
function ToggleRecordAllCalls () {
  ...
  RecordAllCalls = RecordAllCalls !== true
    ...
    selectEvent: function (event, ui) {
      ...
      if (id !== null) {
        ...
        if (id.indexOf('input-') > -1) {
          ...
          let mustRestartRecording = false
          if (session.data.mediaRecorder && session.data.mediaRecorder.state === 'recording') {
            StopRecording(lineNum, true)
        ...
        if (id.indexOf('output-') > -1) {
          ...
          // Remote Audio
          const element = $('#line-' + lineNum + '-remoteAudio').get(0)
          if (element) {
...
function PresentVideo (lineNum) {
  ...
  const html = '<div class="UiWindowField"><input type=file  accept="video/*" id=SelectVideoToSend></div>'
...
function RemoveScratchpad (lineNum) {
  let scratchpad = GetCanvas('line-' + lineNum + '-scratchpad')
  if (scratchpad !== null) {
...
function ReformatMessage (str) {
  ...
  msg = msg.replace(/((([A-Za-z]{3,9}:(?:\/\/)?)(?:[-;:&=\+\$,\w]+@)?[A-Za-z0-9.-]+|(?:www.|[-;:&=\+\$,\w]+@)[A-Za-z0-9.-]+)((?:\/[\+~%\/.\w-_]*)?\??(?:[-\+=&;%@.\w_]*)#?(?:[\w]*))?)/gi, function (x) {
    const niceLink = (x.length > 50) ? x.substring(0, 47) + '...' : x
    const rtn = '<A target=_blank class=previewHyperlink href="' + x + '">' + niceLink + '</A>'
    return rtn
  })
  return msg
}
...
function getPicture (buddy, typestr, ignoreCache) {
  const avatars = defaultAvatars.split(',')
  const rndInt = Math.floor(Math.random() * avatars.length)
  const defaultImg = hostingPrefix + '' + imagesDirectory + '' + avatars[rndInt].trim()
...
function CreateImageEditor (buddy, placeholderImage) {
  ...
  let imgWidth = placeholderImage.width
  let imgHeight = placeholderImage.height
  const maxWidth = $('#contact-' + buddy + '-imagePastePreview').width() - 2 // for the border
  const maxHeight = 480
  $('#contact-' + buddy + '-imageCanvas').prop('width', maxWidth)
  $('#contact-' + buddy + '-imageCanvas').prop('height', maxHeight)

  // Handle Initial Zoom
  let zoomToFitImage = 1
  let zoomWidth = 1
  let zoomHeight = 1
  ...
  const canvas = new fabric.Canvas('contact-' + buddy + '-imageCanvas')
...
function GetCanvas (canvasId) {
  for (let c = 0; c < CanvasCollection.length; c++) {
    try {
...
const ImageEditor_Select = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
    canvas.ToolSelected = 'none'
    canvas.isDrawingMode = false
    return true
...
const ImageEditor_FreedrawPaint = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
...
const ImageEditor_Pan = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
...
const ImageEditor_ResetZoom = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
    canvas.setZoom(1)
...
const ImageEditor_ZoomIn = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
    let zoom = canvas.getZoom()
    zoom = zoom + 0.5
...
const ImageEditor_ZoomOut = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
    let zoom = canvas.getZoom()
    zoom = zoom - 0.5
...
const ImageEditor_AddCircle = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
    canvas.ToolSelected = 'none'
    canvas.isDrawingMode = false
...
const ImageEditor_AddRectangle = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
    canvas.ToolSelected = 'none'
    canvas.isDrawingMode = false
...
const ImageEditor_AddTriangle = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
    canvas.ToolSelected = 'none'
    canvas.isDrawingMode = false
...
const ImageEditor_AddEmoji = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
    canvas.ToolSelected = 'none'
    canvas.isDrawingMode = false
    const text = new fabric.Text(String.fromCodePoint(0x1F642), { fontSize: 24 })
    canvas.add(text)
...
const ImageEditor_AddText = function (buddy, textString) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
    canvas.ToolSelected = 'none'
    canvas.isDrawingMode = false
    const text = new fabric.IText(textString, { fill: canvas.FillColour, fontFamily: 'arial', fontSize: 18 })
    canvas.add(text)
    canvas.centerObject(text)
    canvas.setActiveObject(text)
    return true
...
const ImageEditor_ClearAll = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
...
const ImageEditor_Send = function (buddy) {
  const canvas = GetCanvas('contact-' + buddy + '-imageCanvas')
  if (canvas !== null) {
    const imgData = canvas.toDataURL({ format: 'webp' }) // png
...
// FileShare an Upload
// ===================
const allowDradAndDrop = function () {
...
// ===========
// jQuery UI
function OpenWindowWithButtonArray (html, title, height, width, hideCloseButton, allowResize, buttonConfigs, DoOnLoad, OnClose) {
  ...
  if (buttons.length >= 1) {
    windowObj.dialog('option', 'buttons', buttons)
  }
...
function PopupMenu (obj, menu) {
  ...
  const x = $(obj).offset().left - $(document).scrollLeft()
  const y = $(obj).offset().top - $(document).scrollTop()
  const w = $(obj).outerWidth()
  const h = $(obj).outerHeight()

  menuObj = $('<ul></ul>')
  if (menu && menu.items) {
    $.each(menu.items, function (i, item) {
      const header = (item.isHeader === true) ? ' class="ui-widget-header"' : ''
      const disabled = (item.isDisabled === true) ? ' class="ui-state-disabled"' : ''
      if (item.icon !== null) {
  ...
  // Final Positions
  const menuWidth = menuObj.outerWidth()
  let left = x - ((menuWidth / 2) - (w / 2))
  ...
  const menuHeight = menuObj.outerHeight()
  let top = y + h
...
function XmppSendPing () {
  ...
  const iq_request = $iq({ type: 'get', id: XMPP.getUniqueId(), to: XmppDomain, from: XMPP.jid })
...
function XmppSetMyPresence (str, desc, updateVcard) {
  ...
  const pres_request = $pres({ id: XMPP.getUniqueId(), from: XMPP.jid })
  ...
  if (updateVcard === true) {
    const base64 = getPicture('profilePicture')
    const imgBase64 = base64.split(',')[1]
    const photoHash = $.md5(imgBase64)
...
function onPresenceChange (presence) {
  ...
  const subscription = presence.getAttribute('subscription')
  const type = (presence.getAttribute('type')) ? presence.getAttribute('type') : 'presence' // subscribe | subscribed | unavailable
  let pres = ''
  let status = ''
  let xmlns = ''
  ...
  const buddyObj = FindBuddyByJid(fromJid)
  if (buddyObj === null) {
...
function XmppConfirmSubscription (buddyObj) {
  ...
  const pres_request = $pres({ to: buddyObj.jid, from: XMPP.jid, type: 'subscribed' })
...
function XmppSendSubscriptionRequest (buddyObj) {
  ...
  const pres_request = $pres({ to: buddyObj.jid, from: XMPP.jid, type: 'subscribe' })
  XMPP.sendPresence(pres_request)
  // Responses are handled in the main handler
}
...
function XmppRemoveBuddyFromRoster (buddyObj) {
  ...
  const iq_request = $iq({ type: 'set', id: XMPP.getUniqueId(), from: XMPP.jid })
...
function XmppGetBuddies () {
  ...
  const iq_request = $iq({ type: 'get', id: XMPP.getUniqueId(), from: XMPP.jid })
  iq_request.c('query', { xmlns: 'jabber:iq:roster' })
  ...
  XMPP.sendIQ(iq_request, function (result) {
    ...
    Strophe.forEachChild(result, 'query', function (query) {
      Strophe.forEachChild(query, 'item', function (buddyItem) {
        ...
        const jid = buddyItem.getAttribute('jid')
        const displayName = buddyItem.getAttribute('name')
        const node = Strophe.getNodeFromJid(jid)
        let buddyDid = node
        if (XmppRealm !== '' && XmppRealmSeparator !== '') {
          buddyDid = node.split(XmppRealmSeparator, 2)[1]
        }
        const ask = (buddyItem.getAttribute('ask')) ? buddyItem.getAttribute('ask') : 'none'
        const sub = (buddyItem.getAttribute('subscription')) ? buddyItem.getAttribute('subscription') : 'none'
        const isGroup = (jid.indexOf('@' + XmppChatGroupService + '.') > -1)

        let buddyObj = FindBuddyByJid(jid)
        if (buddyObj === null) {
          // Create Cache
          if (isGroup === true) {
...
function RefreshBuddyData (buddyObj) {
  ...
  // Get Last Activity
  const iq_request = $iq({ type: 'get', id: XMPP.getUniqueId(), to: buddyObj.jid, from: XMPP.jid })
  ...
  XMPP.sendIQ(iq_request, function (result) {
    ...
    if (result.children[0].getAttribute('seconds')) {
      const seconds = Number(result.children[0].getAttribute('seconds'))
...
function XmppGetBuddyVcard (buddyObj) {
  ...
  const iq_request = $iq({ type: 'get', id: XMPP.getUniqueId(), from: XMPP.jid, to: buddyObj.jid })
  iq_request.c('vCard', { xmlns: 'vcard-temp' })
  XMPP.sendIQ(iq_request, function (result) {
    const jid = result.getAttribute('from')
    console.log('Got vCard for: ' + jid)

    const buddyObj = FindBuddyByJid(jid)
    if (buddyObj === null) {
...
function XmppSendMessage (buddyObj, message, messageId, thread, markable, type) {
  ...
  const msg = $msg({ to: buddyObj.jid, type, id: messageId, from: XMPP.jid })
  if (thread && thread !== '') {
...
function XmppStartComposing (buddyObj, thread) {
  ...
  const msg = $msg({ to: buddyObj.jid, from: XMPP.jid })
  if (thread && thread !== '') {
    msg.c('thread').t(thread)
    msg.up()
  }
...
function onPingRequest (iq) {
  ...
  const iq_response = $iq({ type: 'result', id, to: from, from: to })
...
function onInfoQueryRequest (iq) {
  ...
  let query = '' // xml.find("iq").find("query").attr("xmlns")
...
var reconnectXmpp = function () {
  ...
  const xmpp_websocket_uri = 'wss://' + XmppServer + ':' + XmppWebsocketPort + '' + XmppWebsocketPath
  ...
  const xmpp_password = SipPassword
